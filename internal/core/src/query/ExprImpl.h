...
template <typename T>
struct EvalArithmeticOperationExprImpl : EvalArithmeticOperationExpr {
    const T right_operand_;
    const T value_;

    EvalArithmeticOperationExprImpl(const FieldOffset field_offset, const DataType data_type, const ArithOpType arith_op, const T right_operand, const OpType op_type, const T value)
        : EvalArithmeticOperationExpr(field_offset, data_type, op_type, arith_op), right_operand_(right_operand), value_(value) {
    }
};
